<x-app-layout>
    ...
    @else
    <div class="w-screen overflow-auto whitespace-no-wrap py-3 px-4 text-center">
        @foreach($categories as $category)
        <a href="{{route('category.post',['category'=>$category->id])}}" class="font-bold py-2 px-6  rounded hover:underline">
            {{$category->name}}
        </a>
        @endforeach
    </div>
    ...
    <div class="bg-white rounded-lg w-1/2 justify-center sm:justify-start items-center sm:items-start sm:flex-row space-x-2 p-8  ">
      <div class="px-4">
        ...
        <div class="text-justify sm:text-left">
          <div class="pb-3">
            <h1 class=" font-normal text-xl normal-case" style="color: rgb(236 72 153);">{{$post->title}}</h1>
          </div>
          
          <p class="normal-case font-light text-gray-600">
            {{$post->body}}
          </p>
        </div>
